// Returns planar graph representing Delaunay triangulation
// Edges for each vertex are in ccw order.
// To use doubles replace __int128 for double in line 51
...
bool in_c(pto a, pto b, pto c, pto p) {
  // Warning: this number is O(max_coord^4). Consider using
  // doubles or an alternative method for this function
  __int128 p2 = p * p;
  __int128 A = a * a - p2, B = b * b - p2, C = c * c - p2;
  return area(p,a,b)*C + area(p,b,c)*A + area(p,c,a)*B >EPS;
}

pair<Q, Q> build_tr(vector<pto>& p, int l, int r) {
  if (r - l + 1 <= 3) {
    Q a = edge(p[l], p[l + 1], l, l + 1);
    Q b = edge(p[l + 1], p[r], l + 1, r);
  ...
  while (1) {
    ...
    else if (lb->dest().left(lb->o, ra->o))
      lb = lb->rev()->next();
    else break;
  }
  Q b = conn(lb->rev(), ra);
  auto valid = [&](Q e) {
    return b->o.left(e->dest(), b->dest());
  };
  ...
  while (1) {
    ...
    if (!valid(L) ||
        (valid(R) && in_c(L->dest(), L->o,R->o, R->dest())))
      b = conn(R, b->rev());
