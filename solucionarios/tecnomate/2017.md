...
Existe una dinámica que resuelve este problema de manera bastante sencilla (no sabemos si hay otra solución, pero sospechamos que sí, y más eficiente que la nuestra). La función recursiva que implementamos es $ F(i,k) = \sum_(j=i+1)^n {if(a_j>a_i) F(j,k-1)} $
