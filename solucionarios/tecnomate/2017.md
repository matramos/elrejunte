...
Existe una dinámica que resuelve este problema de manera bastante sencilla (no sabemos si hay otra solución, pero sospechamos que sí, y más eficiente que la nuestra). La función recursiva que implementamos es:
```math
   F(i,k) = sum_(j=i+1)^n if(a_j>a_i) {F(j,k-1)}
```
...
-Antes de llamar a F, cargamos un vector para cada elemento que contiene cuales son los elementes que pueden ser siguientes a este.
-Agregamos un caso base que retorna 0 cuando k>n-i